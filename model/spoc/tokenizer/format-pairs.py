    ...
    for i, x in enumerate(text_tokens):
        if x in code_tokens and can_placehold(x):
            ...
            for j, y in enumerate(code_tokens):
                if y == x:
                    code_tokens[j] = '${}'.format(placeholder)
                    replacement_dict['${}'.format(placeholder)] = x
            for j, y in enumerate(text_tokens):
                if y == x:
                    text_tokens[j] = '${}'.format(placeholder)
            placeholder += 1

    return text_tokens, code_tokens, replacement_dict
    ...
    with open(args.infile) as fin:
        ...
        for line in fin:
            ...
            if not args.tokenize_only:
                text_tokens_match, code_tokens_match, replacement_dict = match(text_tokens, code_tokens)

            if args.tokenize_only:
                print('{}\t{}'.format(' '.join(text_tokens), ' '.join(code_tokens)))
            else:
                # 打印替换后的文本和代码，以及替换字典
                # text code text_v code_v replace_dict workerid probid subid line indent
                print('{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}'.format(' '.join(text_tokens),' '.join(code_tokens),' '.join(text_tokens_match), ' '.join(code_tokens_match), replacement_dict,data['workerid'],data['probid'],data['subid'],data['line'],data['indent']))
