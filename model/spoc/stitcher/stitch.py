...
################################################################
# Oracle stuff
# 遍历输入语句和预测语句的对应元素，生成报告。
# 如果预测是虚拟值（'DUMMY'），则将虚拟值写入报告。
# 如果预测不是虚拟值，将预测的文本、最佳预测、黄金标准写入报告。
# 查找黄金标准在预测的候选中的排名，并更新in_beam_exact_match。
# 报告预测分数、黄金标准分数以及黄金标准在候选中的排名。
# 最后，函数返回in_beam_exact_match，该值表示在beam中的所有预测是否都与黄金标准匹配。
...
# FLAG detailed_oracle这个函数就是将每行进行替换，测试功能正确性
    ...
    for inp, pred in zip(inp_stmt, pred_stmt):
        ...
        if tmp_ind < curr_ind:
            ...
            if tmp_ind - curr_ind > 1:
                indent += (tmp_ind - curr_ind - 1) * "{ "
        curr_ind = tmp_ind
        if pred[_pred.text] == 'DUMMY':# 缩进+代码+换行
...
def stitch():
    probno, folder = ARGS.probno, ARGS.folder# probno 问题编号
    count = 0
    inp_stmt, pred_stmt = [], []

    # the following look extracts the input/pred lines for the probno specified
    # and passes it further for stitching以下代码块提取了指定probno的输入/预测行，并将其进一步传递给拼接处理。
    with open(folder + '.tsv','r') as tsvin, open(folder + '.summary','r') as predin:
        ...
        while True:
            inp = tsvin.readline()
            if not inp:
                # Special handling for last line 对最后一行进行特殊处理
                assert count == probno, \
                    'num problems = {} but probno = {}'.format(count, probno)
                break
            inp = inp.split('\t')
            pred = predin.readline().split("\t")
            if int(inp[_inp.line].strip()) == 0: # 查看是否为第一行
                if count == probno:
                    break
                count += 1
