...
@register_model("glat_decomposed_link_no")
class GlatDecomposedLink(FairseqNATModel):

    def __init__(self, args, encoder, decoder):
        super().__init__(args, encoder, decoder)
        # pdb.set_trace()
        # self.args.max_decoder_batch_tokens = 19384
    ...
    def extract_links(self, features, prev_output_tokens,
        ...
        query = query_linear(features_withpos)
        key = key_linear(features_withpos)

        query_chunks = query.reshape(batch_size, seqlen, chunk_num, chunk_size)
        key_chunks = key.reshape(batch_size, seqlen, chunk_num, chunk_size)

        query_chunks = F.normalize(query_chunks, p=2, dim=-1)
        key_chunks = F.normalize(key_chunks, p=2, dim=-1)
  
        log_multi_content = (torch.einsum("bicf,bjcf->bijc", query_chunks.to(dtype=target_dtype), key_chunks.to(dtype=target_dtype)) * (self.decoder.scale))
    ...
    def extract_features(self, prev_output_tokens, encoder_out, net_input, rand_seed, require_links=False, training=True):
        with torch_seed(rand_seed):
            features, attenion = self.decoder.extract_features(
                            prev_output_tokens,
                            net_input,
    ...
    """
    KEY:initialize_output_tokens
        计算上采样后的长度
    """
    # 实例化空数组：<s> <unk> <unk> <unk> </s>
    ...
    def initialize_output_tokens_upsample_by_tokens(self, src_tokens):
        if vars(self.args).get("src_upsample_scale", None) is not None:
            ...
            length_tgt = (length_tgt * self.args.src_upsample_scale).long().clamp_(min=2) # 上采样，src_length*采样系数，并限制最小值为2
        else:
...
class GlatLinkDecoder(NATransformerDecoder):
    ...
    def init_link_feature(self, args):
        ...
        self.output_linear = nn.Linear(args.decoder_embed_dim*2, args.decoder_embed_dim)
        # chunk_num = self.args.decoder_attention_heads
        # chunk_size = self.args.decoder_embed_dim // self.args.decoder_attention_heads
        # self.scale = Parameter(torch.tensor(np.log2((chunk_size**2) - chunk_size)))
        self.scale = Parameter(torch.tensor(12).float())
...
@register_model_architecture(
    "glat_decomposed_link_no", "glat_decomposed_link_no_base"
)
